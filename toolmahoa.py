...
KIỂU 1: OBF THẤP (DÀNH CHO TẤT CẢ TỆP) (DỄ DÀNG ĐẾN DEOBF)
KIỂU 2: TRUNG BÌNH (LỰA CHỌN TỐT NHẤT) (CHUỖI ĐẦY ĐỦ, INT, OBF BOOL)
KIỂU 3: CAO (KHÔNG KHUYẾN NGHỊ) (ĐÂY LÀ CHẾ ĐỘ TRUNG BÌNH NHƯNG X2 SPAM)

CHỐNG HOOKING (KHỎI BẺ)
...
def stage(text: str, symbol: str = 'QUANG HUY', col1 = light, col2 = None) -> str:
    if col2 is None:
        col2 = light if symbol == 'QUANG HUY' else purple
...
_file = input(" NHẬP FILE CẦN MÃ HOÁ: ")
...
while True:
    try:
        ...
        _file = input(" KHÔNG TÌM THẤY FILE ĐÓ: ")
        

while True:
    try:
        mode = int(input(" CHỌN KIỂU MÃ HOÁ: "))
        if mode < 4:
            break
    except ValueError:
        pass
moreobf = input(" BẠN CÓ MUỐN MORE OBF? (y/n): ")

antidebug = input(" BẠN CÓ MUỐN ANTI DEBUG? (y/n): ")

method = input(" BẠN CÓ MUỐN COMPILE? (y/n): ")
...
((
    ((([["QUANG HUY"],
    [" ĐÃ ĐƯỢC MÃ HOÁ BỞI QUANG HUY"],
    ["FB:QUANG HUY(huycayf)"],
   3.11
    ],
...
"""
#mã hoá bởi sếp huy rồi nhé mấy cưng
anti = r"""
...
"""
open("damahoa-" + _file, "w", encoding="utf8").write(str(code))
print(" ĐÃ LƯU VỚI TÊN", "damahoa-" + _file)